...
export class MetaService {
  ...
  setTitle(title: string, override = false): void {
    ...
    title$.subscribe((res: string | undefined) => {
      let fullTitle: string | undefined;

      if (!res) {
        const defaultTitle$ =
          this.settings.defaults && this.settings.defaults.title
            ? this.callback(this.settings.defaults.title)
            : observableOf('');

        defaultTitle$.subscribe((defaultTitle: string | undefined) => {
          if (
            !override &&
            this.settings.pageTitleSeparator &&
            this.settings.applicationName
          ) {
            this.callback(this.settings.applicationName).subscribe(
              (applicationName: string | undefined) => {
                fullTitle = applicationName
                  ? this.getTitleWithPositioning(defaultTitle!, applicationName)
                  : defaultTitle;
                if (defaultTitle) {
        ...
        this.callback(this.settings.applicationName).subscribe(
          (applicationName: string | undefined) => {
            fullTitle = applicationName
              ? this.getTitleWithPositioning(res, applicationName)
              : res;
  ...
  setTag(key: string, value: string): void {
    if (key === 'title') {
      throw new Error(
        `Attempt to set ${key} through "setTag": "title" is a reserved tag name. ` +
    ...
    const cur =
      value ||
      (this.settings.defaults && this.settings.defaults[key]
        ? this.settings.defaults[key]
        : '');

    const value$ =
      key !== 'og:locale' && key !== 'og:locale:alternate'
        ? this.callback(cur)
        : observableOf(cur);

    value$.subscribe((res: string | undefined) => {
      if (res) {
  ...
  update(currentUrl: string, metaSettings?: any): void {
    ...
    if (this.settings.defaults) {
      Object.keys(this.settings.defaults).forEach((key) => {
        ...
        if (
          (metaSettings && (key in this.isMetaTagSet || key in metaSettings)) ||
          key === 'title' ||
          key === 'override'
        ) {
          return;
          ...
          const currentLocale = metaSettings
            ? metaSettings['og:locale']
            : undefined;
          if (value) this.updateLocales(currentLocale, value);
  ...
  private callback(value: string | undefined): Observable<string | undefined> {
  ...
  private getTitleWithPositioning(
    ...
    switch (this.settings.pageTitlePositioning) {
      case PageTitlePositioning.AppendPageTitle:
        return (
          applicationName + String(this.settings.pageTitleSeparator) + title
        );
      case PageTitlePositioning.PrependPageTitle:
        return (
          title + String(this.settings.pageTitleSeparator) + applicationName
        );
      default:
        throw new Error(
          `Invalid pageTitlePositioning specified [${this.settings.pageTitlePositioning}]!`
        );
    }
  }
  ...
  private updateLocales(currentLocale: string, availableLocales: string): void {
    const cur =
      currentLocale ||
      (this.settings.defaults ? this.settings.defaults['og:locale'] : '');
